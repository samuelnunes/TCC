...
UI.Select.prototype.setOptionsArray = function ( options ) {
...
UI.Select.prototype.setArrayOpcoes = function ( opcoes ) {
...
UI.NumberInteger = function ( number ) {
	...
	this.onMouseMove = function ( event ) {
		...
		dom.value = Math.floor(Math.min( scope.max, Math.max( scope.min, number ) ).toFixed( scope.precision ));

		if ( scope.onChangeCallback ) scope.onChangeCallback();
	};
};
UI.NumberInteger.prototype = Object.create( UI.Number.prototype );
